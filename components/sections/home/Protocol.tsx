...
function ProtocolStep({ step, index, total, scrollYProgress }: { 
  ...
  return (
    <div className={`flex flex-col ${step.side === 'right' ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center mb-32 lg:mb-12 will-change-transform`}>
      <div className={`w-full lg:w-1/2 ${step.side === 'right' ? 'lg:pl-20' : 'lg:pr-20'}`}>
        <div className="group relative">
          <div className="absolute -top-12 -left-8 lg:-top-24 lg:-left-20 text-[10rem] lg:text-[18rem] font-black text-zinc-100 dark:text-zinc-900/40 select-none pointer-events-none group-hover:text-zinc-200 dark:group-hover:text-zinc-900 transition-colors duration-1000">
            0{index + 1}
          </div>
          ...
          <motion.div style={{ opacity: opacityProgress }} className="relative z-10 translate-z-0">
            ...
            <motion.h3 
              ...
              className="text-4xl md:text-6xl font-black text-zinc-950 dark:text-white uppercase tracking-tighter mb-6"
            >
              {step.title}
            </motion.h3>
            <motion.p 
              style={{ opacity: colorProgress }}
              className="max-w-md text-xl text-zinc-500 dark:text-zinc-500 font-medium italic leading-relaxed"
            >
              {step.desc}
            </motion.p>
          </motion.div>
...
export default function Protocol() {
  ...
  return (
    <section ref={containerRef} className="py-60 px-6 relative bg-white dark:bg-zinc-950 overflow-hidden transform-gpu">
