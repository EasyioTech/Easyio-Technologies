        ...
        {[...new Array(2)].map((_, index) => (
          <React.Fragment key={index}>
            {props.testimonials.map(({ text, image, name, role }, i) => (
              <div
                className="p-6 md:p-10 rounded-2xl md:rounded-[2rem] border border-zinc-800 bg-zinc-900 shadow-sm transition-colors max-w-xs w-full group"
                key={i}
              >
...
export default function Testimonials({ initialTestimonials }: { initialTestimonials: any[] }) {
  ...
  return (
    <section className="py-16 md:py-40 bg-zinc-950 relative overflow-hidden">
      {/* Background Accent */}
